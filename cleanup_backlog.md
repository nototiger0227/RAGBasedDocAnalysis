...
## AI Chat (High Priority)

### Fix white screen after Company + Year backend update

Reason:
- Backend ChatRequest now requires `year`.
- Frontend still sends only:
  - company
  - question
  - history
- Request validation fails, causing the chat page to render a blank screen.

Tasks:
- Fetch reports using `/dashboard/companies/list`.
- Replace company-only selector with Company (Year) selector.
- Maintain selected report state:
  - company
  - year
- Update chat POST request to send:
  - company
  - year
  - question
  - history
- Verify RAG retrieves chunks using:
  - user_id
  - company
  - year
...
### Fix Tesla 2025 Extraction
- Verify Chroma metadata contains correct year.
- Verify BM25 retrieval filters by year.
- Verify Hybrid Retriever filters by year.
- Verify extract_metric() passes year correctly.
- Re-upload reports after year-aware retrieval changes.
- Confirm different reports return different KPIs.

---

### AI Chat

- Add Company + Year selector.
- Send year with every chat request.
- Update ChatRequest schema.
- Update RAG pipeline to retrieve documents using:
  - User ID
  - Company
  - Year
...
- Backend removes the report.
- Frontend still displays:
  - "Failed to delete report."

Investigate:
- Delete API response.
- Error handling after refreshing report list.
...
- Compare similar metrics instead of mixing vastly different scales
...
- Remove old company-only logic.
- Remove unused state variables.
- Remove obsolete helper functions.
- Verify all APIs consistently use:
  - Company
  - Year
  - User ID