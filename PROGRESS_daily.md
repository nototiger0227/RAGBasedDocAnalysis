...
# Daily Progress - 29 July 2026

## Dashboard Improvements

### Company-Year Based Report Selection
- Updated dashboard report selector to support multiple reports for the same company.
- Backend `/dashboard/companies/list` now returns:
  - Company
  - Financial Year
- Dropdown now displays reports in the format:
  - Apple (2024)
  - Tesla (2025)
- Dashboard now requests data using:
  - `/dashboard/{company}/{year}`
- AI Insights endpoint also updated to:
  - `/dashboard/{company}/{year}/insights`

---

## Financial Metrics Refactor

- Refactored dashboard metrics API.
- Each KPI now returns:
  - Display value
...
```json
{
    "display": "$245.1 billion",
    "value": 245100000000
}
```

This prepares the application for future visualizations and financial calculations while preserving the original extracted values.

---

## Financial Parser

Implemented a reusable parser capable of converting extracted financial values into numeric representations.
...
- Evaluated Recharts implementation.
- Encountered compatibility/runtime issues with React 19.
- Migrated financial visualization to Chart.js.
- Integrated Financial Overview chart into the dashboard.
- Added:
  - Responsive layout
  - Animated bars
...
Started propagating financial year throughout the retrieval pipeline.
...
Every financial request should now retrieve chunks using:
...
Identified a major issue causing incorrect dashboard results.

Root Cause:

Dashboard API route accepted `year` but the database query ignored it and always returned the first matching company.

Fixed database query to include:
...
✅ Dashboard supports Company + Year selection.

✅ Backend routes updated.

✅ Financial parser integrated.

✅ Chart.js integrated.

⚠ Tesla 2025 metrics currently return `NOT_FOUND`.

Investigation shows this is likely related to retrieval filtering or metric extraction rather than the dashboard itself.
